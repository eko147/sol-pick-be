...
public class MealPlanService {
    ...
    public Map<String, Object> getMealPlan(Map<String, Object> userData) {
        ...
        try {
            ...
            // ✅ OpenAI가 생성한 JSON 콘텐츠 추출 및 정규화 처리
            JsonNode messageNode = choicesNode.get(0).path("message").path("content");

            // 응답이 배열 형태인지, 객체 형태인지 확인
            String content = messageNode.asText();
            Map<String, Object> parsed = objectMapper.readValue(content, Map.class);

            // 응답이 이미 meal_plan 배열이면 그대로 반환
            if (parsed.containsKey("meal_plan")) {
                return parsed;
            }

            // 아니라면 day 기반의 map -> meal_plan 배열로 변환
            List<Map<String, Object>> mealPlanList = new ArrayList<>();
            for (Map.Entry<String, Object> entry : parsed.entrySet()) {
                Map<String, Object> meal = (Map<String, Object>) entry.getValue();
                meal.put("day", entry.getKey()); // "Day 1", "Day 2" 등 추가
                mealPlanList.add(meal);
            }

            Map<String, Object> result = new HashMap<>();
            result.put("meal_plan", mealPlanList);
            return result;
    ...
    private String createPrompt(Map<String, Object> userData) {
        ...
        String prompt = String.format(
                "사용자의 정보를 기반으로 맞춤형 6일치 식단을 추천해주세요. "
                        ...
                        + "- 'calories': 대략적인 칼로리 정보\n"
                        + "- 'steps': 조리 과정 설명 (배열, 예: ['닭가슴살을 굽는다', '양파와 토마토를 썬다'])\n\n"
                        + "반환 데이터는 JSON 형식이어야 하며, { \"meal_plan\": [...] } 형식으로 구성해주세요.",
                gender, height, weight, goal, targetWeight, meals, sleepHours, activityLevel, exercise
        );
